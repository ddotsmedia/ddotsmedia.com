...
import { Building2, Clock, Globe, Mail, MapPin, MessageCircle, Phone, type LucideIcon } from "lucide-react";
...
const CARD =
  "flex items-start gap-4 rounded-2xl border border-[#1e2d42] bg-[#111827] p-5 transition-all duration-300 hover:border-[var(--brand-teal)] hover:shadow-[0_0_30px_-12px_var(--brand-teal)]";
const CHIP =
  "inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-[var(--brand-teal)]/30 bg-[var(--brand-teal)]/10 text-[var(--brand-teal)]";

export const metadata: Metadata = {
  title: "Contact",
  description:
    "Get in touch with Ddotsmedia — web, software, mobile, and ERP solutions for the UAE and GCC.",
};
...
export default async function ContactPage() {
  ...
  const wa = (cs?.whatsappNumber ?? "").replace(/[^\d]/g, "");
  const officeLocation = cs?.officeLocation ?? "";
  const social = cs?.socialLinks ?? null;

  const INFO = [
    { icon: Mail, label: "Email", value: email, href: email ? `mailto:${email}` : undefined },
    { icon: Phone, label: "Phone", value: phone, href: phone ? phoneHref : undefined },
    { icon: MapPin, label: "Location", value: officeLocation, href: undefined as string | undefined },
  ].filter((i) => i.value);

  const METHODS: { icon: LucideIcon; label: string; value: string; href: string; external?: boolean }[] = [];
  if (phone) METHODS.push({ icon: Phone, label: "Call us", value: phone, href: phoneHref });
  if (email) METHODS.push({ icon: Mail, label: "Email us", value: email, href: `mailto:${email}` });
  if (wa) METHODS.push({ icon: MessageCircle, label: "WhatsApp", value: "Instant chat", href: `https://wa.me/${wa}`, external: true });
  if (cs?.officeVisitNote) METHODS.push({ icon: Building2, label: "Office visit", value: cs.officeVisitNote, href: "#contact-form" });
  ...
  return (
    <main className="min-h-screen bg-navy">
        ...
        <div className="relative mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
          ...
          {cs?.responseTime && (
            <p className="mx-auto mt-4 inline-flex items-center gap-2 rounded-full border border-[var(--brand-teal)]/30 bg-[var(--brand-teal)]/10 px-4 py-1.5 text-sm text-white/70">
              ...
              We typically respond {cs.responseTime}.
            </p>
          )}
        </div>
      </section>

      {/* CONTACT METHODS STRIP — only when configured in admin */}
      {cs && METHODS.length > 0 && (
      ...
      {/* SPLIT: form + (admin-driven) info */}
      <section className="pb-24">
        <div className={`mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 sm:px-6 lg:px-8 ${cs ? "lg:grid-cols-5" : ""}`}>
          <Reveal className={cs ? "lg:col-span-3" : ""}>
            <ContactForm />
          </Reveal>
          ...
          {cs && (
            <Reveal delay={0.1} className="lg:col-span-2">
              <div className="flex h-full flex-col gap-4">
                {INFO.map((item) => {
                  const inner = (
                    <article className={CARD}>
                      ...
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-widest text-white/40">{item.label}</p>
                ...
                {(cs.officeHoursWeekday || cs.officeHoursFriSat || cs.officeHoursNote) && (
                  <div className="rounded-2xl border border-[#1e2d42] bg-[#111827] p-5">
                    ...
                    <div className="mt-4 space-y-1.5 text-sm text-white/75">
                      {cs.officeHoursWeekday && <p>{cs.officeHoursWeekday}</p>}
                      {cs.officeHoursFriSat && <p className="text-white/55">{cs.officeHoursFriSat}</p>}
                    </div>
                    {cs.officeHoursNote && (
                ...
                {/* Map widget (structural) with admin-driven location label */}
                <div className="relative overflow-hidden rounded-2xl border border-[#1e2d42]">
                  <iframe
                    title="Ddotsmedia location"
      ...
      {/* SOCIAL — only when links are configured */}
      {cs && SOCIAL.length > 0 && (
